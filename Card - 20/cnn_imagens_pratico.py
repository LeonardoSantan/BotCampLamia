...
image = Image.open('./Card - 20/image.png').convert('L')  # Carrega a imagem em escala de cinza
...
input_image = transform_real(image).unsqueeze(0)  # Adiciona dimensão extra para batch

# Faz a predição com o modelo
output = model(input_image)  # Passa a imagem pelo modelo
predicted_class = output.argmax(dim=1).item()  # Obtém a classe prevista
